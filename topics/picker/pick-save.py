...
if True:
    ...
    def _delete_pickup_files(path: str) -> None:
        for folder_path in _get_path_list(path, "*"):
    ...
    class Pick:
        ...
        @classmethod
        def main(cls):
            _delete_pickup_files("pickup-topic")
            pathlist = _get_path_list("topics/", "*")
            shuffle(pathlist)
            for pick_folder_path in pathlist[:3]:
                cls._copy_pickup_files(pick_folder_path)
                print(pick_folder_path)

    # save
    class Save:
        def _copy_pickup_files(folder_path):
            target_folder_path = os.path.join("topic", os.path.basename(folder_path))
            shutil.copytree(folder_path, target_folder_path)

        @classmethod
        def main(cls):
            pathlist = _get_path_list("pickup-topic/", "*")
            for pick_folder_path in pathlist[:3]:
                cls._copy_pickup_files(pick_folder_path)
                print(pick_folder_path)
